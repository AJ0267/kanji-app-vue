...
<script>
import Navbar from "../components/Navbar.vue";
import n5Data from "@/assets/N5KanjiData.json";
import n4Data from "@/assets/N4KanjiData.json";
import n3Data from "@/assets/N3KanjiData.json";
import n2Data from "@/assets/N2KanjiData.json";
import n1Data from "@/assets/N1KanjiData.json";
...
export default {
  ...
  methods: {
    async fetchKanjiData() {
      ...
      try {
        const response = await fetch(`/${kanjiFiles[this.selectedLevel]}`);
        if (response.ok) {
          const data = await response.json();
          this.kanjiData = data[this.selectedLevel] || [];
          this.currentPage = 1;
          this.searchQuery = "";
        } else {
          console.error("Failed to fetch Kanji data:", response.statusText);
        }
      } catch (error) {
